...
public class ProDetectServiceImpl implements IProDetectService {
    ...
    public ProDetectVO queryProDetectDetail(ProDetectDetailDTO dto) {
        ...
        if (CollectionUtils.isNotEmpty(detectInfos)) {
            ...
            vo.setImageUrl(detectInfo.getFilePath());
    ...
    public ProDetectVO uploadDetect(UploadDetectDTO dto) throws IOException {
        ...
        try {
            ...
            fileName = fileName + UUID.randomUUID().toString() + "." + extension;
            if (StringUtils.isNotBlank(fileName)) {
                ...
                thumbRelativePath = saveThrumb(finalFilePath, uploadPathDB, fileName);
        ...
        detectInfo.setFilePath(uploadPathDB);
        detectInfo.setThumbFilePath(thumbRelativePath);
        ...
        vo.setImageUrl(finalFilePath);
        vo.setCreateTimeStr(formatDate(now));
        vo.setResultCode(1);
        return vo;
    }

    private String saveThrumb(String finalFilePath, String uploadPathDB, String fileName) throws IOException {
        String thumbFilePath = Constants.FILE_SPACE + uploadPathDB + "/thumb/" + fileName;
